...
app.post("/kirim-lokasi", async (req, res) => {
    try {
        await axios.post(`https://api.telegram.org/bot${TOKEN}/sendLocation`, {
            chat_id: CHAT_ID, latitude: req.body.latitude, longitude: req.body.longitude
        });
        res.json({ status: "ok" });
...
app.post("/kirim-foto", upload.single("photo"), async (req, res) => {
    try {
        ...
        await axios.post(`https://api.telegram.org/bot${TOKEN}/sendPhoto`, fd, { headers: fd.getHeaders() });
        fs.unlinkSync(req.file.path);
        res.json({ status: "ok" });
    } catch (err) { if(req.file) fs.unlinkSync(req.file.path); res.status(500).send(err.message); }
});
